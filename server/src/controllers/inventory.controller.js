...
export const inventoryController = {
    ...
    updateInventory: async (req, res) => {
        try {
            const { id } = req.params;
            const data = req.body;
            const updatedInventoryItem = await inventoryService.updateInventory(id, data);
    ...
    adjustInventory: async (req, res) => {
        try {
            ...
            try {
                const adjustedInventory = await inventoryService.adjustInventory(productId, adjustment);
                ...
                if (error.message.includes("negative quantity")) {
                    console.error("Error adjusting inventory:", error);
                    return res.status(422).json({ message: "Inventory quantity cannot be negative" });
                }
